...
export const confirmSalesOrder = async (req, res) => {
    try {
        ...
        for (const item of salesOrder.items) {
            await reserveStock(item.item, item.quantity, salesOrder._id, "SalesOrder", req.user._id);
    ...
    try {
        ...
        if (salesOrder.isConfirmed) {
            for (const item of salesOrder.items) {
                ...
                if (unreservedQty > 0) {
                    await releaseStock(item.item, unreservedQty, salesOrder._id, "SalesOrder", req.user._id);
                }
            }
        }
