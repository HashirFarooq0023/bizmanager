...
const ForgotPassword = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-black text-slate-900 dark:text-white font-sans selection:bg-violet-600 selection:text-white transition-colors duration-300 relative overflow-hidden flex flex-col justify-between p-3 sm:p-6 md:p-8">
      ...
      <div className="absolute -top-32 -left-32 w-72 sm:w-96 md:w-[500px] h-72 sm:h-96 md:h-[500px] bg-violet-500/15 dark:bg-violet-600/20 blur-[140px] rounded-full pointer-events-none max-w-[100vw]" />
      <div className="absolute -bottom-32 -right-32 w-72 sm:w-96 md:w-[500px] h-72 sm:h-96 md:h-[500px] bg-indigo-500/15 dark:bg-indigo-600/20 blur-[150px] rounded-full pointer-events-none max-w-[100vw]" />

      {/* Top Header Bar */}
      <div className="w-full max-w-md mx-auto flex items-center justify-between z-10 mb-4 sm:mb-6 gap-2">
        <Link
          to="/login"
          className="inline-flex items-center gap-1 sm:gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition font-urdu"
        >
          <svg className={`w-4 h-4 ${isRtl ? 'scale-x-[-1]' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
          </svg>
          <span className="truncate max-w-[130px] sm:max-w-none">{t('auth:forgot.backToLogin')}</span>
        </Link>
      ...
      <div className="max-w-md w-full mx-auto relative z-10 my-auto px-1 sm:px-0 py-4 sm:py-8">
        ...
        <div className="bg-white/85 dark:bg-[#0C0F17]/95 rounded-2xl sm:rounded-3xl shadow-xl dark:shadow-2xl dark:shadow-black/90 border border-slate-200/80 dark:border-zinc-800/80 p-5 sm:p-8 backdrop-blur-xl transition-colors duration-200">
          {(isError || isSuccess) && (
            <div className={`mb-5 sm:mb-6 p-3.5 sm:p-4 border rounded-xl flex items-center gap-2.5 font-urdu ${isSuccess ? 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-900/60' : 'bg-rose-50 dark:bg-rose-950/40 border-rose-200 dark:border-rose-900/60'}`}>
              <p className={`${isSuccess ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300'} text-xs sm:text-sm font-semibold`}>{message}</p>
            </div>
          )}
          ...
          <form onSubmit={onSubmit} className="space-y-5 sm:space-y-6">
            ...
            >
              {isLoading ? t('auth:forgot.sending') : t('auth:forgot.submitButton')}
            </button>
          </form>

          <div className="mt-5 sm:mt-6 text-center font-urdu">
      ...
      <div className="mt-6 text-center text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-urdu px-2">
        {language === 'ur' ? (
