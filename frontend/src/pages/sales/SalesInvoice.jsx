    ...
    <Layout>
      <div className="space-y-6">
        ...
        <PageHeader
          ...
          description="View, filter and manage customer sales transactions and billing records."
          actions={
            <Button
        ...
        {isError && (
          <div className="p-3 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 rounded-xl">
            <p className="text-rose-700 dark:text-rose-400 text-xs font-medium">{message}</p>
          </div>
        )}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-1 text-[8px]">
                      <div className="flex items-center gap-1 text-gray-500 dark:text-[rgb(var(--color-text-secondary))] flex-1 min-w-0">
                        <span className="truncate">{new Date(invoice.createdAt).toLocaleDateString("en-PK")}</span>
                        <span>•</span>
                        <span>{invoice.items.length}i</span>
                        <span>•</span>
                        <span className="truncate">{invoice.paymentMethod}</span>
                        <span>•</span>
                        <span className="truncate">P:Rs. {invoice.paidAmount.toFixed(0)}</span>
                      </div>
                  ...
                  <tbody className="bg-white dark:bg-[rgb(var(--color-table-row))] divide-y divide-gray-200 dark:divide-[rgb(var(--color-border))]">
                    {paginatedInvoices.map((invoice) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-xs text-gray-900 dark:text-[rgb(var(--color-text))] capitalize">
